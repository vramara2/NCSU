...
module tb_top ();
  ...
  //------------------weights and bias-----------------------------------------                                          
  wire [11:0]                              dut__fmem__read_address    ;
  wire [15:0]                              fmem__dut__read_data       ;  // read data
                                          
  wire [11:0]                              dut__gmem__read_address    ;
  wire [15:0]                              gmem__dut__read_data       ;  // read data

  wire [11:0]                              dut__imem__read_address    ;
  wire [15:0]                              imem__dut__read_data       ;  // read data
  
  wire [11:0]                              dut__omem__read_address    ;
  wire [15:0]                              omem__dut__read_data       ;  // read data

  //-----------------------Tanh LUT--------------------------------------------
  wire [11:0]                              dut__tanhmem__read_address ;
  wire [15:0]                              tanhmem__dut__read_data    ;   // read data

  wire tanhmem__dut__read_enable;
  wire gmem__dut__read_enable;
  wire imem__dut__read_enable;
  wire omem__dut__read_enable;
  wire fmem__dut__read_enable;
  ...
  sram  #(.ADDR_WIDTH    (12),
          ...
          .read_enable  ( sram__dut__read_enable   ), 
          .clock        ( clk                      )
         );
  ...
  sram  #(.ADDR_WIDTH    (12),
          ...
          .write_address( 12'b0                    ),
          .write_data   ( 16'b0                    ), 
          .read_address ( dut__fmem__read_address  ),
          .read_data    ( fmem__dut__read_data     ),
          .read_enable  ( fmem__dut__read_enable   ), 
          .clock        ( clk                      )
         );
  ...
  sram  #(.ADDR_WIDTH    (12),
          ...
          .read_enable  ( gmem__dut__read_enable   ), 
          .clock        ( clk                      )
         );

  //sram for i weights and bias
  sram  #(.ADDR_WIDTH    (12),
          .DATA_WIDTH    (16),
          .MEM_INIT_FILE ("i.dat"           ))
          imem_mem  (
          ...
          .write_address( 12'b0                    ),
          .write_data   ( 16'b0                    ), 
          .read_address ( dut__imem__read_address  ),
          .read_data    ( imem__dut__read_data     ), 
          .read_enable  ( imem__dut__read_enable   ), 
          .clock        ( clk                      )
         );

  //sram for o weights and bias
  sram  #(.ADDR_WIDTH    (12),
          .DATA_WIDTH    (16),
          .MEM_INIT_FILE ("o.dat"           ))
          omem_mem  (
          ...
          .write_address( 12'b0                    ),
          .write_data   ( 16'b0                    ), 
          .read_address ( dut__omem__read_address  ),
          .read_data    ( omem__dut__read_data     ), 
          .read_enable  ( omem__dut__read_enable   ), 
          .clock        ( clk                      )
         );
  ...
  sram  #(.ADDR_WIDTH    (12),
          ...
          .read_enable  ( tanhmem__dut__read_enable   ), 
          .clock        ( clk                      )
         );
...
//SRAM interface
          ...
          .sram__dut__read_enable    ( sram__dut__read_enable      ),
//---------------------------------------------------------------------------
//weights and biases interface                                                       
          .dut__fmem__read_address    ( dut__fmem__read_address    ),
          .fmem__dut__read_data       ( fmem__dut__read_data       ),
          .fmem__dut__read_enable     (  fmem__dut__read_enable    ),
          
          .dut__gmem__read_address    ( dut__gmem__read_address    ),
          .gmem__dut__read_data       ( gmem__dut__read_data       ),
          .gmem__dut__read_enable     (  gmem__dut__read_enable    ),
          
          .dut__imem__read_address    ( dut__imem__read_address    ),
          .imem__dut__read_data       ( imem__dut__read_data       ),
          .imem__dut__read_enable    ( imem__dut__read_enable      ),
          
          .dut__omem__read_address    ( dut__omem__read_address    ),
          .omem__dut__read_data       ( omem__dut__read_data       ),
          .omem__dut__read_enable    ( omem__dut__read_enable      ),
          
          .dut__tanhmem__read_address ( dut__tanhmem__read_address ),
          .tanhmem__dut__read_data    ( tanhmem__dut__read_data    ),
          .tanhmem__dut__read_enable  ( tanhmem__dut__read_enable  )
  ...
  initial begin
      ...
      for(j=0;j<ROUND;j=j+1) begin
         ...
         fmem_mem.loadInitFile($sformatf("input_and_output_%0d/f.dat",j));
         gmem_mem.loadInitFile($sformatf("input_and_output_%0d/g.dat",j));
         imem_mem.loadInitFile($sformatf("input_and_output_%0d/i.dat",j));
         omem_mem.loadInitFile($sformatf("input_and_output_%0d/o.dat",j));
  ...
  // Compare your compute results with the results compute by MATLAB
       ...
       for(i=0;i<256;i=i+1) begin
        ...
        //if(gresult_array[i+256][15:16-VALID]==golden_gresult_array[i][15:16-VALID]) begin
        if(($itor(gresult_array[i+256])*sf) - ($itor(golden_gresult_array[i])*sf) <= 0.0078 && ($itor(gresult_array[i+256])*sf) - ($itor(golden_gresult_array[i])*sf) >= -0.0078) begin
	 correctGnumber[q]=correctGnumber[q]+1;
         end
         //else if(golden_gresult_array[i][15] == 0)
         else
         begin
         $display("Miss match in value at : %0d", i);
         $display("expected value: %d", golden_gresult_array[i][15:16-VALID]);
         $display("actual value: %d", gresult_array[i + 256][15:16-VALID]);
         end
       end
       $display("-------------------------------Round %0d Your report-------------------------------\n",q);
       $display("Correct g results = %0d/256",correctGnumber[q]);
